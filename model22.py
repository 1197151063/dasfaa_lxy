...
class Model(nn.Module):
    ...
    def _init_graph(self):
        if self.A_split:
            self.Graph_1 = self._split_A_hat(self.adj_1, self.n_users_1, self.n_items_1)
            self.Graph_2 = self._split_A_hat(self.adj_2, self.n_users_2, self.n_items_2)
        else:
            self.Graph_1 = self._convert_sp_mat_to_sp_tensor(self.adj_1)
            self.Graph_1 = self.Graph_1.coalesce().to(self.device)
            self.Graph_2 = self._convert_sp_mat_to_sp_tensor(self.adj_2)
    ...
    def computer(self, graph, users_emb, items_emb, n_users, n_items, uuMat, iiMat, uiMat, norm=1):
        ...
        uu_embed0 = self.self_gatingu(users_emb)  # 通过自主门控机制，派生出每个因子空间的uu和ii
        ii_embed0 = self.self_gatingi(items_emb)
        ...
        for i in range(len(self.encoder)):
            ...
            if i == 0:
                ...
                userEmbeddings0 = layer(userEd, uuMat, user_index)
                itemEmbeddings0 = layer(itemEd, iiMat, item_index)

            for j in range(n_factors_l):
                if self.A_split:
                    ...
                else:
                    all_embs.append(t.sparse.mm(g_droped, all_embs_tp[j]))

            layer_embs.append(all_embs)
            factor_embedding = t.cat([all_embs[0], all_embs[1]], dim=1)
            all_emb = factor_embedding  # 用户-项目交互
            self.all_ui_embeddings += [factor_embedding]
            ...
            if norm == 1:  # 选择是否对新嵌入进行 L2 归一化操作
                ...
                self.all_user_embeddings += [userEmbeddings0]
    ...
    ***********************************************************
    ...
    def forward(self, users_1, pos_items_1, neg_items_1, users_2, pos_items_2, neg_items_2, userindex_S, itemindex_S, userindex_T, itemindex_T, train):
        ...
        self.ua_embeddings_1, self.ia_embeddings_1, self.layer_embeddings_1, self.userEmbedding_1, self.itemEmbedding_1 = self.computer(self.Graph_1,
                                                self.embedding_dict['user_embedding_S'], self.embedding_dict['item_embedding_S'], self.n_users_1,
                                                self.n_items_1, self.uuMat_1, self.iiMat_1, self.adj_1)
        self.ua_embeddings_2, self.ia_embeddings_2, self.layer_embeddings_2, self.userEmbedding_2, self.itemEmbedding_2 = self.computer(self.Graph_2,
                                                self.embedding_dict['user_embedding_T'], self.embedding_dict['item_embedding_T'], self.n_users_2,
                                                self.n_items_2, self.uuMat_2, self.iiMat_2, self.adj_2)
        ...
        if train == True:  # 只有在训练模式下才会计算元路径正则化损失。
            ...
            ssl_loss_HetG = ssl_loss_S + ssl_loss_T
    ...
    def get_embeddings(self, users_1, pos_items_1, neg_items_1, users_2, pos_items_2, neg_items_2):
       ...
       self.neg_i_g_embeddings_2 = t.index_select(self.final_i_2, 0, t.LongTensor(neg_items_2).to(self.device))
        ...
        # 以未融合uu和ii前的U I进行测试
        #  self.u_g_embeddings_1 = t.index_select(self.ua_embeddings_1, 0, t.LongTensor(users_1).to(self.device))
        #  self.pos_i_g_embeddings_1 = t.index_select(self.ia_embeddings_1, 0, t.LongTensor(pos_items_1).to(self.device))
        #  self.neg_i_g_embeddings_1 = t.index_select(self.ia_embeddings_1, 0, t.LongTensor(neg_items_1).to(self.device))
        # #
        #  self.u_g_embeddings_2 = t.index_select(self.ua_embeddings_2, 0, t.LongTensor(users_2).to(self.device))
        #  self.pos_i_g_embeddings_2 = t.index_select(self.ia_embeddings_2, 0, t.LongTensor(pos_items_2).to(self.device))
        #  self.neg_i_g_embeddings_2 = t.index_select(self.ia_embeddings_2, 0, t.LongTensor(neg_items_2).to(self.device))
