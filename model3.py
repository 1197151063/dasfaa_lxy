...
# 版本2，将损失函数的计算全部一到哦了model中，并且将self.embedding_dict['user_embedding_S']作为HGCL的输入（即初始化嵌入），同时舍去HGCL中对ui图的操作。
...
class Model(nn.Module):
    ...
    def __init__(self, config, args, device):
        ...
        self.adj_1 = config['adj_mat_S']   # 无用
        self.adj_2 = config['adj_mat_T']
        ...
        self.gating_weightub = nn.Parameter(t.FloatTensor(1, hide_dim//2))  # 是一个可学习的模型参数，它是一个形状为 (1, hide_dim) 的权重矩阵
        nn.init.xavier_normal_(self.gating_weightub.data)  # 将其初始化为 Xavier 初始化的随机值
        self.gating_weightu = nn.Parameter(t.FloatTensor(hide_dim//2, hide_dim//2))
        nn.init.xavier_normal_(self.gating_weightu.data)
        self.gating_weightib = nn.Parameter(t.FloatTensor(1, hide_dim//2))
        nn.init.xavier_normal_(self.gating_weightib.data)
        self.gating_weighti = nn.Parameter(t.FloatTensor(hide_dim//2, hide_dim//2))
    ...
    def computer(self, users_emb, items_emb, n_users, n_items, uuMat, iiMat, uiMat, norm=1):
        item_index = np.arange(0, n_items)  # 创建一个物品索引数组
        user_index = np.arange(0, n_users)  # 创建一个用户索引数组
        ...
        self.all_ui_embeddings = [all_emb]
        layer_embs = []
        inner_uu = []
        inner_ii = []
        ...
        for i in range(len(self.encoder)):
            ...
            n_factors_l = factor_num[i]
            all_embs_tp = t.split(all_emb, int(self.emb_dim / n_factors_l), 1)  # 有两个因子，第一个代表领域不变空间，第二个代表领域特定空间。
            all_embs = []  # 原先的 all_embs = []
            all_embs_uu = []
            all_embs_ii = []
            for j in range(n_factors_l):
                ui_embed0 = all_embs_tp[j]
                if self.A_split:
                    ...
                else:
                    if i == 0:
                        # 取出每一个因子空间中的U和I
                        all_embs_tp_u, all_embs_tp_i = ui_embed0[:n_users], ui_embed0[n_users:]
                        uu_embed0 = self.self_gatingu(all_embs_tp_u)  # 通过自主门控机制，派生出每个因子空间的uu和ii
                        ii_embed0 = self.self_gatingi(all_embs_tp_i)
                        inner_uu.append(uu_embed0)
                        inner_ii.append(ii_embed0)

                        userEmbeddings0 = layer(uu_embed0, uuMat, user_index)
                        itemEmbeddings0 = layer(ii_embed0, iiMat, item_index)
                        uiEmbeddings0 = layer(ui_embed0, uiMat, ui_index)
                    else:
                        # 如果不是第一层（即 else 分支），则使用前一层的嵌入作为输入，再次应用当前层的 GCN。
                        uu_embed0 = t.split(userEd, int(self.emb_dim / n_factors_l), 1)  # 也将uu和ii划分为领域不变和领域特定的两个子空间
                        ii_embed0 = t.split(itemEd, int(self.emb_dim / n_factors_l), 1)

                        userEmbeddings0 = layer(uu_embed0[j], uuMat, user_index)
                        itemEmbeddings0 = layer(ii_embed0[j], iiMat, item_index)
                        uiEmbeddings0 = layer(ui_embed0, uiMat, ui_index)

                    all_embs.append(uiEmbeddings0)
                    all_embs_uu.append(userEmbeddings0)
                    all_embs_ii.append(itemEmbeddings0)

            # 存储每层的嵌入结果ui
            if i == 0:
                self.all_user_embeddings = [t.cat([inner_uu[0], inner_uu[1]], dim=1)]  # 初始化了用户和物品的嵌入列表，将初始嵌入添加到列表中。
                self.all_item_embeddings = [t.cat([inner_ii[0], inner_ii[1]], dim=1)]
            else:
                continue

            layer_embs.append(all_embs)
            factor_embedding = t.cat([all_embs[0], all_embs[1]], dim=1)
            factor_embedding_uu = t.cat([all_embs_uu[0], all_embs_uu[1]], dim=1)
            factor_embedding_ii = t.cat([all_embs_ii[0], all_embs_ii[1]], dim=1)

            self.ui_userEmbedding0, self.ui_itemEmbedding0 = t.split(factor_embedding, [n_users, n_items])
            userEd = (factor_embedding_uu + self.ui_userEmbedding0) / 2.0  # 将当前层处理后的用户嵌入与联合嵌入的用户部分求平均
            itemEd = (factor_embedding_ii + self.ui_itemEmbedding0) / 2.0
            all_emb = t.cat([userEd, itemEd], 0)  # 用户-项目交互

            if norm == 1:  # 选择是否对新嵌入进行 L2 归一化操作
                norm_embeddings = F.normalize(factor_embedding_uu, p=2, dim=1)
                self.all_user_embeddings += [norm_embeddings]
                norm_embeddings = F.normalize(factor_embedding_ii, p=2, dim=1)
                self.all_item_embeddings += [norm_embeddings]
                norm_embeddings = F.normalize(factor_embedding, p=2, dim=1)
                self.all_ui_embeddings += [norm_embeddings]
            else:
                self.all_user_embeddings += [userEd]
                self.all_item_embeddings += [norm_embeddings]
                self.all_ui_embeddings += [norm_embeddings]
        ...
        # 用来计算正负样本的
        users, items = t.split(self.uiEmbedding, [n_users, n_items])
        finall_layer = layer_embs[-1]

        metatsuembed, metatsiembed = self.metafortansform(uiMat, n_users, self.userEmbedding, users, self.itemEmbedding, items)
        self.userEmbedding = self.userEmbedding + metatsuembed
        self.itemEmbedding = self.itemEmbedding + metatsiembed

        return users, items, finall_layer, self.userEmbedding, self.itemEmbedding

    # 执行元路径知识抽取和个性化转换  self.userEmbedding, self.ui_userEmbedding, self.itemEmbedding, self.ui_itemEmbedding
    def metafortansform(self, uiMat, n_users, auxiembedu, U, auxiembedi, I):
        uiadj, iuadj = self.neighbor_information(uiMat, n_users)

        # Neighbor information of the target node  这两行代码计算了用户和物品的邻居信息。
        uneighbor = t.matmul(uiadj.to(self.device), I)  # 包含了用户的物品邻居的信息
        ineighbor = t.matmul(iuadj.to(self.device), U)  # 包含了物品的用户邻居的信息

        # Meta-knowlege extraction  这两行代码使用元路径注意力模型来抽取用户和物品的元路径知识
        tembedu = (self.meta_netu(t.cat((auxiembedu, U, uneighbor), dim=1).detach()))  # 附加的领域信息显式地增强了直接图连接的建模
        tembedi = (self.meta_neti(t.cat((auxiembedi, I, ineighbor), dim=1).detach()))
    ...
    ***********************************************************
    ...
    def predictModel(self,user, pos_i, neg_j, isTest=False):
        if isTest:
            pred_pos = t.sigmoid(t.sum(user * pos_i, dim=1)).to(self.device)
            return pred_pos
        else:
            pred_pos = t.sigmoid(t.sum(user * pos_i, dim=1)).to(self.device)
            pred_neg = t.sigmoid(t.sum(user * neg_j, dim=1)).to(self.device)
            return pred_pos.to(self.device), pred_neg.to(self.device)
    ...
    def forward(self, users_1, pos_items_1, neg_items_1, users_2, pos_items_2, neg_items_2, userindex_S, itemindex_S, userindex_T, itemindex_T, train):
        ...
        self.ua_embeddings_1, self.ia_embeddings_1, self.layer_embeddings_1, self.userEmbedding_1, self.itemEmbedding_1 = self.computer(
                                                self.embedding_dict['user_embedding_S'], self.embedding_dict['item_embedding_S'], self.n_users_1,
                                                self.n_items_1, self.uuMat_1, self.iiMat_1, self.uiMat_1)
        self.ua_embeddings_2, self.ia_embeddings_2, self.layer_embeddings_2, self.userEmbedding_2, self.itemEmbedding_2 = self.computer(
                                                self.embedding_dict['user_embedding_T'], self.embedding_dict['item_embedding_T'], self.n_users_2,
                                                self.n_items_2, self.uuMat_2, self.iiMat_2, self.uiMat_2)

        self.final_u_1 = self.wu1 * self.ua_embeddings_1 + self.wu2 * self.userEmbedding_1
        self.final_i_1 = self.wu1 * self.ia_embeddings_1 + self.wu2 * self.itemEmbedding_1
        self.final_u_2 = self.wu1 * self.ua_embeddings_2 + self.wu2 * self.userEmbedding_2
        self.final_i_2 = self.wu1 * self.ia_embeddings_2 + self.wu2 * self.itemEmbedding_2

        # 根据索引从不同的嵌入矩阵中提取出对应的用户和正样本和负样本的嵌入向量，以及预训练的......
        # self.pred_pos_S, self.pred_neg_S = self.predictModel(self.final_u_1[users_1], self.final_i_1[pos_items_1], self.final_i_1[neg_items_1])
        # self.pred_pos_T, self.pred_neg_T = self.predictModel(self.final_u_1[users_1], self.final_i_1[pos_items_1], self.final_i_1[neg_items_1])

        self.u_g_embeddings_1 = t.index_select(self.final_u_1, 0, t.LongTensor(users_1).to(self.device))
        self.pos_i_g_embeddings_1 = t.index_select(self.final_i_1, 0, t.LongTensor(pos_items_1).to(self.device))
        self.neg_i_g_embeddings_1 = t.index_select(self.final_i_1, 0, t.LongTensor(neg_items_1).to(self.device))

        self.u_g_embeddings_2 = t.index_select(self.final_u_2, 0, t.LongTensor(users_2).to(self.device))
        self.pos_i_g_embeddings_2 = t.index_select(self.final_i_2, 0, t.LongTensor(pos_items_2).to(self.device))
        self.neg_i_g_embeddings_2 = t.index_select(self.final_i_2, 0, t.LongTensor(neg_items_2).to(self.device))


        metaregloss = 0.  # 元路径正则化损失
        regLoss = 0.
        bpr_loss = 0.
        ssl_loss_MI = 0.
        DCCloss = 0.
        ...
        if train == True:  # 只有在训练模式下才会计算元路径正则化损失。
            # 计算bpr损失
            # mf_loss_1 = - nn.LogSigmoid()(self.pred_pos_S - self.pred_neg_S).sum()
            # mf_loss_2 = - nn.LogSigmoid()(self.pred_pos_T - self.pred_neg_T).sum()
            # bpr_loss = mf_loss_1 + mf_loss_2
            mf_loss_1 = self.create_bpr_loss(self.u_g_embeddings_1, self.pos_i_g_embeddings_1,
                                             self.neg_i_g_embeddings_1)
            mf_loss_2 = self.create_bpr_loss(self.u_g_embeddings_2, self.pos_i_g_embeddings_2,
            ...
            # ssl_loss_MI = self.alpha * ssl_loss
            DCCloss = mf_loss_1 + mf_loss_2 + self.alpha * ssl_loss
            # 计算元路径损失
            metaregloss_1 = self.calculate_metaregloss(self.ua_embeddings_1[userindex_S.cpu().numpy()], self.userEmbedding_1, self.ia_embeddings_1[itemindex_S.cpu().numpy()],
                                                       self.itemEmbedding_1, self.uuMat_1[userindex_S.cpu().numpy()], self.iiMat_1[itemindex_S.cpu().numpy()])
            metaregloss_2 = self.calculate_metaregloss(self.ua_embeddings_2[userindex_T.cpu().numpy()], self.userEmbedding_2, self.ia_embeddings_2[itemindex_T.cpu().numpy()],
                                                       self.itemEmbedding_2, self.uuMat_2[userindex_T.cpu().numpy()], self.iiMat_2[itemindex_T.cpu().numpy()])
            metaregloss = metaregloss_1 + metaregloss_2
            # 计算异构图对比损失
            ssl_loss_user_S = self.ssl_loss(self.ua_embeddings_1, self.userEmbedding_1, users_1)
            ssl_loss_user_T = self.ssl_loss(self.ua_embeddings_2, self.userEmbedding_2, users_2)
            ssl_loss_item_S = self.ssl_loss(self.ia_embeddings_1, self.itemEmbedding_1, pos_items_1)
            ssl_loss_item_T = self.ssl_loss(self.ia_embeddings_2, self.itemEmbedding_2, pos_items_2)
            ...
            ssl_loss_HetG = 0.5 * ssl_loss_S + 0.5 * ssl_loss_T

            # 计算regloss
            regLoss_S = (t.norm(self.final_u_1[users_1]) ** 2 + t.norm(self.final_i_1[pos_items_1]) ** 2 + t.norm(self.final_i_1[neg_items_1]) ** 2)
            regLoss_T = (t.norm(self.final_u_1[users_1]) ** 2 + t.norm(self.final_i_1[pos_items_1]) ** 2 + t.norm(self.final_i_1[neg_items_1]) ** 2)
            regLoss = 0.5 * regLoss_S + 0.5 * regLoss_T

        trainneed = {}
        trainneed['metaregloss'] = metaregloss
        trainneed['ssl_loss_HetG'] = ssl_loss_HetG
        trainneed['DCCloss'] = DCCloss
        trainneed['regLoss'] = regLoss
        # trainneed['ssl_loss_MI'] = ssl_loss_MI
        # trainneed['bpr_loss'] = bpr_loss
        return trainneed
