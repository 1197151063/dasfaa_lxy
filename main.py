    ...
    def overall_operation(self, config):
        ...
        for epoch in range(args.epoch):
            ...
            for batch_idx in range(num_batches):
                ...
                loss1 = (train_result['regLoss_S'] * args.reg / args.batch_size) + train_result['ssl_loss_S'] * args.ssl_beta + train_result['metaregloss_1'] * args.metareg
                loss2 = (train_result['regLoss_T'] * args.reg / args.batch_size) + train_result['ssl_loss_T'] * args.ssl_beta + train_result['metaregloss_2'] * args.metareg
                total_loss = 0.45 * loss1 + 0.45 * loss2 + 0.1 * train_result['variationloss']
                total_loss = 0.1 * loss1 + 0.1 * loss2 + 0.8 * train_result['variationloss']

                epoch_loss += train_result['variationloss'].item() / num_batches
            ...
            progress_bar.close()
            # 打印每个 epoch 的平均损失
            print(f'Epoch parsers: time:{time.time()-t1}s, Average Loss: {epoch_loss:.4f}')
