...
class Model(nn.Module):
    ...
    def _init_graph(self):
        if self.A_split:
            self.Graph_1 = self._split_A_hat(self.adj_1, self.n_users_1, self.n_items_1, self.device)
            self.Graph_2 = self._split_A_hat(self.adj_2, self.n_users_2, self.n_items_2, self.device)
        else:
    ...
    # 异构信息传播和聚合
    def information_dissemination_aggregation(self, n_users, n_items, ua_embeddings, ia_embeddings, U, I, uuMat, iiMat, uiMat, norm = 1):
        # HGCL
        ...
        for i in range(len(self.encoder)):
            ...
            userEmbeddings = userEmbeddings0 # 将当前层处理后的用户嵌入与联合嵌入的用户部分求平均
        ...
        # ！！！最终嵌入！！！
        metatsuembed, metatsiembed = self.metafortansform(uiMat, n_users, self.userEmbedding, U, self.itemEmbedding, I)
...
# 不按照批次[uid.cpu().numpy()]计算元路径正则化损失
    ...
    def forward(self, users_1, pos_items_1, neg_items_1, users_2, pos_items_2, neg_items_2, userindex_S, itemindex_S, userindex_T, itemindex_T, train):
        ...
        # 融合了异构信息之后得到的用户、物品、layer嵌入向量
        self.userEmbedding_1, self.itemEmbedding_1 = self.information_dissemination_aggregation(self.n_users_1,
                                                                                                self.n_items_1,
                                                                                                self.embedding_dict['user_embedding_S'],
                                                                                                self.embedding_dict['item_embedding_S'],
        ...
        self.userEmbedding_2, self.itemEmbedding_2 = self.information_dissemination_aggregation(self.n_users_2,
                                                                                                self.n_items_2,
                                                                                                self.embedding_dict['user_embedding_T'],
                                                                                                self.embedding_dict['item_embedding_T'],
                                                                                                self.ua_embeddings_2,
                                                                                                self.ia_embeddings_2,
                                                                                                self.uuMat_2,
                                                                                                self.iiMat_2,
                                                                                                self.adj_2)  # 若是uiMat效果可以
